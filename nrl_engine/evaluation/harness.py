...
from nrl_engine.evaluation.metrics import compute_all_metrics
...
class EvaluationHarness:
    ...
    def _prepare_data(self) -> None:
        ...
        self.data = (
            self.data.dropna(subset=["match_id", "date"])
            .sort_values("date")
        ...
        has_odds = (
            "home_odds_close" in self.data.columns
            and "away_odds_close" in self.data.columns
        )

        if has_odds:
            self.data, self.odds_report = enforce_odds_orientation(
                self.data, config=self.config, verbose=True
            )
        else:
    ...
    def run_evaluation(
        ...
        if fold_type == "rolling":
            ...
            folds = create_anchored_folds(
                self.dataset, test_seasons=test_seasons, config=self.config
            )
        ...
        if "brier" in brier:
            print(
                f"  Brier: {brier['brier']:.4f} (skill: {brier.get('brier_skill', 0):.3f})"
            )
