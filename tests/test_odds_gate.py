...
def test_swapped_odds_detected():
    ...
    data_swapped["home_odds_close"], data_swapped["away_odds_close"] = (
        data["away_odds_close"].copy(),
        data["home_odds_close"].copy(),
    )
    ...
    assert result[
        "swapped_healthy"
    ], "Swapped data should have healthy swapped orientation"
    assert not result[
        "as_is_healthy"
    ], "Swapped data should not have healthy as-is orientation"
...
def test_auto_fix_swapped_odds():
    ...
    fixed_data, report = enforce_odds_orientation(
        data_swapped, config=config, verbose=False
    )
    ...
    pd.testing.assert_series_equal(
        fixed_data["home_odds_close"], original_home_odds, check_names=False
    )
...
def test_market_slope_calculation():
    ...
    assert (
        slope_result["slope"] > 0
    ), f"Slope should be positive, got {slope_result['slope']}"
    assert (
        slope_result["brier"] < 0.25
    ), f"Brier should be < 0.25, got {slope_result['brier']}"
