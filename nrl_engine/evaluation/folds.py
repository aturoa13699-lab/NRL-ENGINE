...
def create_anchored_folds(
    ...
    for test_season in test_seasons:
        ...
        if len(train_seasons) < min_train_seasons:
            print(
                f"  Skipping {test_season}: only {len(train_seasons)} train seasons (need {min_train_seasons})"
            )
            continue
        ...
        if len(test_df) < min_test_matches:
            print(
                f"  Skipping {test_season}: only {len(test_df)} test matches (need {min_test_matches})"
            )
            continue
        ...
        print(
            f"  Fold {fold_id}: Train {min(train_seasons)}-{max(train_seasons)} ({len(train_df)}), "
...
def create_rolling_folds(
    ...
    for test_season in test_seasons:
        ...
        if len(prior_seasons) < train_window:
            print(
                f"  Skipping {test_season}: only {len(prior_seasons)} prior seasons (need {train_window})"
            )
            continue
...
def auto_detect_test_seasons(
    df: pd.DataFrame, min_matches_per_season: int = 50, n_test_seasons: int = 3
) -> list[int]:
    """
    ...
    valid_seasons = season_counts[
        season_counts >= min_matches_per_season
    ].index.tolist()
    valid_seasons = sorted(valid_seasons)

    # Return most recent n seasons
    return (
        valid_seasons[-n_test_seasons:]
        if len(valid_seasons) >= n_test_seasons
        else valid_seasons
    )
