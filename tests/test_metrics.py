...
def test_compute_brier():
    ...
    # Perfect predictions
    df = pd.DataFrame(
        {"pred_home_win_prob": [1.0, 0.0, 1.0, 0.0], "home_win": [1, 0, 1, 0]}
    )
    ...
    # Worst predictions
    df = pd.DataFrame(
        {"pred_home_win_prob": [0.0, 1.0, 0.0, 1.0], "home_win": [1, 0, 1, 0]}
    )
    result = compute_brier(df)
    ...
    df = pd.DataFrame(
        {"pred_home_win_prob": [0.5, 0.5, 0.5, 0.5], "home_win": [1, 0, 1, 0]}
    )
    result = compute_brier(df)
...
def test_compute_auc():
    ...
    # Perfect separation
    df = pd.DataFrame(
        {"pred_home_win_prob": [0.9, 0.8, 0.2, 0.1], "home_win": [1, 1, 0, 0]}
    )
...
def test_compute_calibration():
    ...
    df = pd.DataFrame({"pred_home_win_prob": probs, "home_win": outcomes})
