...
def enforce_odds_orientation(
    ...
    df_swapped = df.copy()
    df_swapped[[home_odds_col, away_odds_col]] = df_swapped[[away_odds_col, home_odds_col]].values
    m_swapped = _compute_market_slope(df_swapped, home_odds_col, away_odds_col, outcome_col)
    ...
    if verbose:
        ...
        for name, m in [("AS_IS", m_as_is), ("SWAPPED", m_swapped)]:
            if "error" in m:
                ...
                print(f"{name:<10} {m['brier']:.4f}     {slope_str:<12} {m['correlation']:.4f}")
    
    # Determine health
    def is_healthy(m: Dict) -> bool:
        return (
            "error" not in m and 
            m.get("slope") is not None and 
            m["slope"] > config.odds_min_healthy_slope
        )
    ...
    if swapped_healthy and not as_is_healthy:
        if verbose:
            print(f"  🚨 Odds SWAPPED upstream (as_is slope={m_as_is.get('slope')}, swapped slope={m_swapped['slope']:.4f})")
        ...
        if config.odds_auto_fix:
            ...
            if verbose:
                print(f"  ⚠️ Auto-fix disabled. Manual fix required.")
    ...
    if as_is_healthy and swapped_healthy:
        ...
        if s2 > s1:
            if verbose:
                print(f"  ⚠️ Both slopes positive; SWAPPED is better -> using SWAPPED")
            report["chosen"] = "swapped"
            report["action"] = "auto_swapped" if config.odds_auto_fix else "manual_fix_needed"
            return (df_swapped if config.odds_auto_fix else df), report
        else:
            if verbose:
                print(f"  ⚠️ Both slopes positive; AS_IS is better -> using AS_IS")
            report["chosen"] = "as_is"
            report["action"] = "none"
            return df, report
    ...
    if verbose:
        print(f"  🚨 Neither orientation yields positive market slope!")
        print(f"  → Likely causes: join corruption, label mismatch, or odds attached to wrong matches")
